...
public class ReclamationService {
    ...
    public void ajouter(Reclamation reclamation) {
        ...
        String sql = "INSERT INTO reclamation (title, description, created_at, type_reclamation_id, fingerprint_verified, user_id) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            LOGGER.info("User ID: {}", reclamation.getUserId());
            ...
            pstmt.setInt(6, reclamation.getUserId());
    ...
    public List<Reclamation> getByUserId(int userId) {
        LOGGER.info("Récupération des réclamations pour l'utilisateur avec ID: {}", userId);
        List<Reclamation> reclamations = new ArrayList<>();
        if (connection == null) {
            LOGGER.error("La connexion est nulle!");
            connection = MyConnection.getInstance().getConnection();
        ...
        String sql = "SELECT r.*, tr.name as type_name FROM reclamation r LEFT JOIN type_reclamation tr ON r.type_reclamation_id = tr.id WHERE r.user_id = ? ORDER BY r.created_at DESC";
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            ...
            while (rs.next()) {
                count++;
                LOGGER.info("Lecture de la réclamation #{} pour user {}", count, userId);
                Reclamation reclamation = new Reclamation();
                reclamation.setId(rs.getInt("id"));
                reclamation.setTitle(rs.getString("title"));
                reclamation.setDescription(rs.getString("description"));
            ...
            LOGGER.error("Erreur lors de la récupération des réclamations par userId: {}", e.getMessage(), e);
        } finally {
            if (rs != null) try { rs.close(); } catch (SQLException e) { }
