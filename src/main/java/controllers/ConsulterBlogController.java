                    ...
                    reportButton.setOnAction(e -> {
                        try {
                            ...
                            if (commentCRUD.hasUserReported(comment.getId(), currentUserId)) {
                                javafx.scene.control.Alert alreadyAlert = new javafx.scene.control.Alert(
                                    javafx.scene.control.Alert.AlertType.INFORMATION,
                                    "Vous avez déjà signalé ce commentaire.",
                                    javafx.scene.control.ButtonType.OK
                                );
                                alreadyAlert.setTitle("Signalement déjà effectué");
                            ...
                            if (comment.getReportCount() == 2) {
                                ...
                                alert.showAndWait().ifPresent(response -> {
                                    if (response == javafx.scene.control.ButtonType.YES) {
                                        try {
                                            commentCRUD.reportCommentByUser(comment.getId(), currentUserId);
                                            loadComments();
