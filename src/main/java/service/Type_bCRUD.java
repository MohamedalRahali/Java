...
public class Type_bCRUD {
    ...
    public void add(Type_b type) throws SQLException {
        String sql = "INSERT INTO type_b (libelle, artist_id) VALUES (?, ?)";
        try (PreparedStatement ps = cnx.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, type.getLibelle());
            ps.setInt(2, type.getArtistId());
            ps.executeUpdate();
    ...
    public void update(Type_b type) throws SQLException {
        String sql = "UPDATE type_b SET libelle = ?, artist_id = ? WHERE id = ?";
    ...
    public List<Type_b> getTypesByArtist(int artistId) throws SQLException {
        List<Type_b> results = new ArrayList<>();
        String sql = "SELECT * FROM type_b WHERE artist_id = ? ORDER BY libelle";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
